...
class SensorFrameCorrector(Node):
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        # 1. Parse raw data as flat uint8 array, then shape it by point rows
        raw_bytes = np.frombuffer(msg.data, dtype=np.uint8)
        num_points = msg.width * msg.height
        
        # Reshape so each row represents exactly one point entry in memory
        point_matrix = raw_bytes.reshape(num_points, msg.point_step)

        # 2. Extract X, Y, Z float coordinates assuming standard offsets (0, 4, 8)
        # This directly pulls out the 4-byte float data for spatial layout
        x_coords = point_matrix[:, 0:4].view(dtype=np.float32).flatten()
        y_coords = point_matrix[:, 4:8].view(dtype=np.float32).flatten()
        z_coords = point_matrix[:, 8:12].view(dtype=np.float32).flatten()

        # 3. Create homogeneous coordinates [X, Y, Z, 1] for rotation
        ...
        # 4. Apply the rotation matrix: P_robot_init = T_rot * P_camera_init
        transformed_points = np.dot(self.T_rot, points.T).T

        # 5. Write the modified spatial coordinates directly back into raw memory view
        # This edits the buffer seamlessly without touching any padding or intensity fields
        point_matrix[:, 0:4] = transformed_points[:, 0].astype(np.float32).view(np.uint8).reshape(-1, 4)
        point_matrix[:, 4:8] = transformed_points[:, 1].astype(np.float32).view(np.uint8).reshape(-1, 4)
        point_matrix[:, 8:12] = transformed_points[:, 2].astype(np.float32).view(np.uint8).reshape(-1, 4)

        # 6. Reconstruct the PointCloud2 message container
        corrected_cloud = PointCloud2()
        corrected_cloud.header.stamp = msg.header.stamp
        corrected_cloud.header.frame_id = 'robot_init'  # Safe right-side up destination frame
        ...
        corrected_cloud.data = point_matrix.tobytes()

        # 7. Publish to CMU pipeline
